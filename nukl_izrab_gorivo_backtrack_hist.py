...
and backtracks the runaway neutrons to their original position
to determine their spatial distribution in z-direction.
...
#-------------------------------------------------------------
pobegle_koordinate = np.load("pobegle_koordinate_fin00g.npy")
# lahko zdruzim vec seznamov ...
# pobegle_koordinate=np.array([np.array([30,30,20])])
# for abcd in pobegle_koordinate0:
#     if abcd[2] > 10:
#         pobegle_koordinate = np.append(pobegle_koordinate, [abcd], axis=0)


xo, yo, zo = np.transpose(seznam_orig)[0], np.transpose(seznam_orig)[1], np.transpose(seznam_orig)[2]
xi, yi, zi = np.transpose(pobegle_koordinate)[0], np.transpose(pobegle_koordinate)[1], np.transpose(pobegle_koordinate)[2]
...
(bins, hist, patches) = plt.hist(zi,  bins=15)
...
x2 = np.linspace(-20, -0.5, 1000)
x = np.linspace(0, 18.5, num=1000)
...
popt, pcov = opt.curve_fit(fitexp, ydata=bins, xdata=(hist + 0.5 * (hist[1] - hist[0]))[:-1])
...
plt.suptitle(f"Bazen z izrabljenimi gorivnimi elementi - izvor ubežnikov")
plt.title("st. generacij: %s, st. potez: %s, $\lambda$: %s, $\mu$: %s "%(100, stevilo_potez, 1.0, mi_abs))
