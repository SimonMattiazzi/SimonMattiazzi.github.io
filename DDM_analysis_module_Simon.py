...
It works for various types of analysis, for examples see "DDM_single_analysis.py" and "DDM_multimeasurement_analysis_2.py"

!!! WARNING !!!: this was once a well-structured module, but it has now become full of pathces and partial and temporary
solutions and it is not as easy to navigate.

Some parts of documentation are not up to date.

To-do list:
- make an optional input parameter "folder" to fit_corr function so you don't need to add "if fit already exists ..."
to every function that relies on fit_corr.
- make an option to save just a fit of a single point in k-space into its coordinates in 2D array.
- add all fit parameters to return parameters of fit_corr
(currently there are several options for different scenarios,
you only need tau for using old fits,
other parameters for initial estimates loop,
popt_2D  and pcov_2D for plotting fits from existing data ...
it would be best if all worked only with popt_2D and pcov_2D)
- write a comprehensive instrucions/documentation of what which function does, which hidden obstacles there are etc.

Written by Simon (00386 41 816 809, simon.mattiazzi@hotmail.com).
