...
as a function of pool surface depth.
...
stevilo_potez = 60
# seznam_x = np.linspace(0, 10, num=30)  # gladine
# seznam_y = []  # preziveli
# for x in seznam_x:
#     bazen_z = x + elem_z
#     seznam = (konstrukcija())
...
# np.save("seznam_x_gl.npy", seznam_x)
# np.save("seznam_y_gl.npy", seznam_y)


seznam_x = np.load("seznam_x_gl.npy")
seznam_y = np.load("seznam_y_gl.npy")
...
x_zv = np.linspace(0, 10, num=1000)
popt, pcov = opt.curve_fit(fun1, seznam_x, seznam_y, p0=[5000,2])


fig = plt.figure(dpi=200)
plt.plot(x_zv, fun1(x_zv, *popt), label="exp fit: $y_0$=%5.3f, $d$=%5.3f"%tuple(popt))  #  risanje grafa tvoje funkcije z optimalnimi parametri
